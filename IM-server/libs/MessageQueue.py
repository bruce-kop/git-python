...
class MQLocal(MessageQueueBase):
    ...
    def put(self, item, block=True, timeout=None):
        ...
        try:
            ...
            logger.debug(f"put to queue faild.{err.with_traceback()}")
            raise err
    ...
    def get(self,block=True, timeout=None):
        ...
        '''
        try:
            return self.msg_queue.get( block=True, timeout=10)
        except Exception as err:
            logger.debug(f"there are no request data, queue is empty.{err}")
            return None,None
